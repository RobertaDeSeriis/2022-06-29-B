...
public class ItunesDAO {
	...
	public List<Album> getAllAlbums(double p){
		final String sql = "SELECT distinct  a.*, (Sum(t.Milliseconds)/1000) AS p "
		...
		try {
			...
			while (res.next()) {
				result.add(new Album(res.getInt("AlbumId"), res.getString("Title"), res.getDouble("p")));
			}
			conn.close();
